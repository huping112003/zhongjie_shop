...
function user_list()
{
    ...
    if ($result === false)
    {
        ...
        $filter['record_count'] = $GLOBALS['db']->getOne("SELECT COUNT(*) FROM ". $GLOBALS['ecs']->table('users') ." as eu right join wxch_qr as wq on wq.affiliate = eu.user_id  " . $ex_where." and eu.user_id is not null");
...
/*         $sql = "SELECT eu.user_id,wu.affiliate, eu.user_name, eu.email, eu.is_validated, eu.user_money, eu.frozen_money, eu.rank_points, eu.pay_points, eu.reg_time FROM ecs_users  eu,wxch_user wu where (wu.wxid = eu.wxid )ORDER by " . $filter['sort_by'] . ' ' . $filter['sort_order'] .
			...
			$sql = "SELECT * FROM  ". $GLOBALS['ecs']->table('users') ."  as eu right join wxch_qr as wq on wq.affiliate = eu.user_id ".$ex_where." and eu.user_id is not null  ORDER by " . $filter['sort_by'] . ' ' . $filter['sort_order'] .
                " LIMIT " . $filter['start'] . ',' . $filter['page_size'];
