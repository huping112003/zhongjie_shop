...
if ($_REQUEST['act'] == 'task')
{
    ...
    else
    {
        ...
        /* 插入code到shop_config表 */
        $sql = "SELECT COUNT(*) FROM " .$ecs->table('touch_shop_config'). " WHERE code = 'integrate_code'";

        if ($db->GetOne($sql) == 0)
        {
            $sql = "INSERT INTO " .$ecs->table('touch_shop_config'). " (code, value) ".
                    "VALUES ('integrate_code', '$_SESSION[code]')";
        }
        else
        {
            $sql = "UPDATE " .$ecs->table('touch_shop_config'). " SET value = '$_SESSION[code]' WHERE code = 'integrate_code'";
        }
        $db->query($sql);
...
if ($_REQUEST['act'] == 'setup_ucenter')
{
    ...
    $app_type   = 'ECSHOP';
    $app_name   = $db->getOne('SELECT value FROM ' . $ecs->table('touch_shop_config') . " WHERE code = 'shop_name'");
...
if ($_REQUEST['act'] == 'edit_points')
{
    ...
    $sql = "UPDATE " . $ecs->table('touch_shop_config') . " SET value ='" . serialize($rule) . "' WHERE code='points_rule'";
...
if ($_REQUEST['act'] == 'save_points')
{
    ...
    $sql = "SELECT COUNT(*) FROM " . $ecs->table('touch_shop_config') . " WHERE code='points_set'";
    if ($db->getOne($sql) == 0)
    {
        $sql = "INSERT INTO " . $ecs->table('touch_shop_config') . " (parent_id, type, code, value) VALUES (6, 'hidden', 'points_set', '" . serialize($cfg) . "')";
    }
    else
    {
        $sql = "UPDATE " . $ecs->table('touch_shop_config') . " SET value ='" . serialize($cfg) . "' WHERE code='points_set'";
    }
    $db->query($sql);
    clear_cache_files();
    sys_msg($_LANG['save_ok'],0, array(array('text'=>$_LANG['06_list_integrate'],'href'=>'integrate.php?act=list')));
...
function save_integrate_config ($code, $cfg)
{
    ...
    if ($GLOBALS['db']->GetOne($sql) == 0)
    {
        $sql =  "INSERT INTO " . $GLOBALS['ecs']->table('touch_shop_config') . " (code, value) ".
                "VALUES ('integrate_config', '" . serialize($cfg) . "')";
    }
    else
    {
        $sql = "UPDATE " . $GLOBALS['ecs']->table('touch_shop_config') . " SET value='". serialize($cfg) ."' ".
