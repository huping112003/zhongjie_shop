...
if ($_REQUEST['act'] == 'optimize')
{
    ...
    while ($row = $db->fetchRow($ret))
    {
        ...
        $list[] = array('table' => $name, 'type' => $type, 'rec_num' => $row['Rows'], 'rec_size' => sprintf(" %.2f KB", $row['Data_length'] / 1024), 'rec_index' => $row['Index_length'],  'rec_chip' => $row['Data_free'], 'status' => $res['Msg_text'], 'charset' => $charset);
