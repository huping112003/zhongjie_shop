...
{
    if (strpos($domain, 'cub168.com.tw') !== false && preg_match('/q=([^&]*)/i', $path, $regs))
    {
    ...
    if (strpos($domain, 'cub168.com') !== false && preg_match('/q=([^&]*)/i', $path, $regs))
    {
        $searchengine = 'GOOGLE';
...
function license_info()
{
    if($GLOBALS['_CFG']['licensed'] > 0)
    {
        ...
        $license = '<a href="http://www.cub168.com/license.php?product=ecshop_b2c&url=' . urlencode($host) . '" target="_blank"
>&nbsp;&nbsp;Licensed</a>';
        return $license;
    }
    else
