        ...
        foreach ($data AS $line)
        {
            ...
            for ($i = 0; $i < $len; $i++)
            {
                ...
                elseif (',' == $char)
                {
                    if (0 == $quote)
                    {
                        if (!isset($field_list[count($arr)]) || $field_list[count($arr)] == 'attr')
                        {
                            if($line_number == 1){
                                $arr['attr'] .= trim($buff) . ',';
                            }
                        ...
                        if(empty($buff) && in_array($field_name, array('is_on_sale', 'is_alone_sale', 'is_real'))){
                            $buff = 1;
                        }
                        if(empty($buff) && $field_name == 'shop_price'){
                            $buff = $arr['market_price'];
                        }
            ...
            $arr['attr'] = $arr['attr'] ? substr($arr['attr'], 0, -1) : '';
            $goods_list[] = $arr;
        }
        //商品属性id
        if($goods_list){
            $attr_name = explode(',', $goods_list[0]['attr']);
            if($attr_name){
                foreach($attr_name as $k=>$val){
                    $row = explode('|', $val);
                    $attr_id .= $row[1]. ',';
                }
