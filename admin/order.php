...
/*------------------------------------------------------ */
//-- 导出订单列表  订单号  下单时间  商品名称  价格 收货人名称  收货人详细地址  收货人联系方式
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'download'){
    ...
    $data = "订单号 \t 下单时间 \t 收货人姓名 \t 收货人联系方式 \t 收货人配送地址 \t 收货人详细地址 \t 订单商品 \n";
    $sql = 'select oi.order_sn, oi.add_time, oi.consignee, oi.mobile, oi.country, oi.province, oi.city, oi.district, oi.address, og.goods_name from' .$ecs->table('order_info') .'AS oi ' .' LEFT JOIN ' .$ecs->table('order_goods') .'AS og ON oi.order_id = og.order_id';
    $region_sql = 'select * from '.$ecs->table('region');
    $region = $db->getAll($region_sql);
    $reg = array();
    foreach($region as $val){
        $reg[$val['region_id']] = $val['region_name'];
    }
    if($day == 1){
        // 获取今日开始时间戳和结束时间戳
        $beginToday=mktime(0,0,0,date('m'),date('d'),date('Y'));
        $endToday=mktime(0,0,0,date('m'),date('d')+1,date('Y'))-1;
        $sql .= " WHERE oi.add_time >= $beginToday AND oi.add_time <= $endToday";
    }
    if($day == -1){
        //php获取昨日起始时间戳和结束时间戳
        $beginYesterday=mktime(0,0,0,date('m'),date('d')-1,date('Y'));
        $endYesterday=mktime(0,0,0,date('m'),date('d'),date('Y'))-1;
        $sql .= " WHERE oi.add_time >= $beginYesterday AND oi.add_time <= $endYesterday";
    }
    ...
    if(!empty($month)){
        ...
        $sql .= " WHERE oi.add_time >= $beginThismonth AND oi.add_time <= $endThismonth";
    }
    if(!empty($start_date)){
        $start_date = strtotime($start_date);
        if(empty($end_date)){
            $end_date = mktime(0,0,0,date('m'),date('d')+1,date('Y'))-1;
        }else{
            $end_date = strtotime($end_date);
        }
        $sql .= " WHERE oi.add_time >= $start_date AND oi.add_time <= $end_date";
    }
    $order = $db->getAll($sql);
    foreach ($order as $val){
        $val['add_time']    = date('Y-m-d H:i:s', $val['add_time']);
        $val['country']     = $reg[$val['country']];
        $val['province']    = $reg[$val['province']];
        $val['city']        = $reg[$val['city']];
        $val['district']    = $reg[$val['district']];
        $val['order_sn'] = 'order:'.$val['order_sn'];
        $data .= $val['order_sn']. " \t ". $val['add_time'] . " \t ". $val['consignee'].
        " \t ". $val['mobile'] . " \t ".
        $val['country'] .' , '. $val['province'] .' , ' . $val['city'] . ' , ' . $val['district'] ." \t ".
        $val['address'] . " \t ". $val['goods_name'] ."\n";
    }
