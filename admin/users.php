...
if ($_REQUEST['act'] == 'list')
{
    ...
    $smarty->assign('action_link2',  array('text' => $_LANG['04_users_add'], 'href'=>'users.php?act=add'));
    $smarty->assign('action_link',  array('text' => '导入会员', 'href'=>'user_export.php?act=add'));
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    $sale = empty($_POST['sale']) ? '' : $_POST['sale'];
    ...
    if(!empty($sale)){
        ...
        $sale_sql = "INSERT INTO ". $ecs->table('term_meta') . " (`object_type`, `object_group`, `object_id`, `meta_key`, `meta_value`) VALUES ('cub-user','user',' $user_id ', 'sale', ' $sale')";
        $db->query($sale_sql);
    }
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    u.office_phone, u.home_phone, u.mobile_phone, u.salesman, u.company".
        " FROM " .$ecs->table('users'). " u LEFT JOIN " . $ecs->table('users') . " u2 ON u.parent_id = u2.user_id WHERE u.user_id='$_GET[id]'";
...
//        $user['formated_frozen_money'] = price_format(0);
    ...
    // 查询销售人员
    $sale_sql = "SELECT meta_value FROM" . $ecs->table('term_meta'). " where `object_type` = 'cub-user' AND `object_id` = $_GET[id] AND `meta_key` = 'sale' ";
    $sale = $db->getone($sale_sql);
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    // 增加销售人员字段
    if(!empty($sale)){
        $count_sql = "SELECT COUNT(*) FROM" . $ecs->table('term_meta') ."WHERE `object_id` = $user_id AND `object_type` = 'cub-user' AND `meta_key` = 'sale' AND `object_group` = 'user'";
        $count = $db->getone($count_sql);
        if(!empty($count)){
            $sale_sql = "UPDATE ". $ecs->table('term_meta') . " SET `meta_value` = '$sale' WHERE `object_id` = $user_id AND `object_type` = 'cub-user' AND `meta_key` = 'sale' AND `object_group` = 'user'";
        }else{
            $sale_sql = "INSERT INTO ". $ecs->table('term_meta') . " (`object_type`, `object_group`, `object_id`, `meta_key`, `meta_value`) VALUES ('cub-user','user',' $user_id ', 'sale', ' $sale')";
        }
        $db->query($sale_sql);
