...
if ($act == 'jiangtang') {
    $smarty->display('jiangtang.dwt');
    die;
} elseif ($act == 'xiaoshou') {
    $smarty->display('xiaoshou.dwt');
    die;
} elseif ($act == 'tejia') {
    $smarty->display('tejia.dwt');
    die;
}
...
if (!$smarty->is_cached('index1.dwt', $cache_id))
{
    ...
    $company_article = $GLOBALS['db']->getrow($sql);
    $company_article['content'] = strip_tags($company_article['content']);
